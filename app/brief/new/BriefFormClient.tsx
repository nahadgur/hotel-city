...
export function BriefFormClient() {
  ...
  // Prefill from ?city=... or ?hotel=... params
  useEffect(() => {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      const url = new URL(data.briefUrl)
  ...
  return (
    <form onSubmit={submit} className="max-w-3xl">
      ...
      {/* Who you are */}
      <div className="mb-10 pt-8 hairline">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          <div>
            <label htmlFor="travellerName" className="eyebrow mb-2 block">
              Your name
            </label>
            <input
              id="travellerName"
              ...
              onChange={(e) => update('travellerName', e.target.value)}
              placeholder="For the hotels to address"
              className="w-full px-4 py-3 bg-paper-50 border border-ink-900/15 focus:border-ink-900 outline-none text-base"
            />
          </div>

          <div>
            <label htmlFor="travellerEmail" className="eyebrow mb-2 block">
              Your email
            </label>
            <input
              id="travellerEmail"
              ...
              onChange={(e) => update('travellerEmail', e.target.value)}
              placeholder="We'll email you each time a quote arrives"
              className="w-full px-4 py-3 bg-paper-50 border border-ink-900/15 focus:border-ink-900 outline-none text-base"
            />
          </div>
        </div>
        <p className="mt-4 text-xs text-ink-500 italic max-w-reading">
          Your email isn't shared with any hotel until you accept a quote. We route replies through
          Stayward so you stay anonymous until you're ready.
        </p>
      </div>
      ...
      <div className="flex items-center gap-4">
        <button type="submit" disabled={submitting} className="btn-primary">
          {submitting ? (
            ...
            <>
              <span>Send brief to hotels</span>
        ...
        <p className="text-xs text-ink-500 italic max-w-xs">
          We'll pick the 5 hotels that fit best and email each one. Quotes usually start arriving
          within a few hours.
        </p>
      </div>
    </form>
